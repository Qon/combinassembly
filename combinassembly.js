let codes = explode` # this is a comment, starts with the '#' and ends when this line does.
:init
##     stack_setup
:main
...
${['#', '']}.global sp, stack 1                     # we with no recursion and only a single function call our stack only needs space to save return address to main
...
function make_entity_composition(assembled) {
  ...
  ;({x} = position_entities(arr_line_ip, x, y))
  ...
  let REG_WIDTH = 3 * (1 + (registers_count > 9))
